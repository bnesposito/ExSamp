# To update documentation devtools::document()
#                        devtools::build_manual()
# To load the package devtools::load_all()
# To run tests:
...
#' @return A list with the distribution and parameters of the posterior distribution of theta.
#' @export
bayesianHierarchicalBinomial = function(outcome, treatment_matrix, group_indicator, n_trials, link = 'logit', control = list()) {

  #TODO: add argument for priors

  strata_encoding = generateEncoding(group_indicator)
  strata_encoded = encodeLabels(group_indicator, strata_encoding)

  data_list = list(N = length(outcome),
                   P = dim(treatment_matrix)[2],
                   J = length(unique(strata_encoded)),
                   x = as.matrix(treatment_matrix),
                   y = outcome,
                   jj = strata_encoded,
                   maxT = n_trials)

  model = stan(file ='./models/binomial_linear_hierarchical.stan',
               data = data_list,
               iter = 2000, warmup = 1000, chains = 4, seed = 1234,
               refresh = 1000)

  posterior_draws = as.matrix(model, pars = 'beta')
...
#' @return A list with the distribution and parameters of the posterior distribution of theta.
#' @export
bayesianBinomial = function(outcome, treatment_matrix, link = 'logit', control = list()) {
  ...
  model = stan(file ='./models/binomial_logit.stan',
               data = data_list,
               iter = 2000, warmup = 1000, chains = 4, seed = 1234,
               refresh = 1000)
