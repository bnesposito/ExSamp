...
plotAverageOutcome = function(history_df, n_waves, cumulative = T, font_size = 20) {
...
plotPosteriorDistributions = function(theta_matrix, treatment_encoding, font_size = 20) {
  ...
  ggplot(theta_melt, aes(x = Theta, fill= Treatment)) +
    ...
    #theme_minimal(base_size = font_size) +
    scale_fill_brewer(palette="Set1") +
    labs(y = "Posterior Density", x = 'Theta', color = 'Treatments')
}
