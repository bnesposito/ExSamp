data {
  ...
  vector<lower=0>[J] sigma;    // standard error of outcomes
}
transformed data {
...
parameters {
  ...
  vector[K*J] eta;             // school level errors
}
...
model {
  ...
  for (n in 1:N)
    target += normal_lpdf(y[n] | theta[(kk[n]-1)*J+jj[n]], sigma[jj[n]]); // log-likelihood
}
generated quantities {
  ...
  for (n in 1: N) {
    y_rep[n] = normal_rng(theta_rep[(kk[n]-1)*J+jj[n]], sigma[jj[n]]);
  }
